...
load('h5ad/R/B_milo_prep_1.5.RData')
# 1.5 MIU vs Untreated (B)
mylo1 <- Milo(adata_no_knnx1)
milo_graph1 <- buildFromAdjacency(knn_adjacencyx1, k=50, is.binary=TRUE)
...
mylo1 <- countCells_(mylo1, meta.data = data.frame(colData(mylo1)), samples="sample_id")
mylo1 <- calcNhoodDistance(mylo1, d=50, reduced.dim = 'X_pca_harmony', use.assay = 'counts')
mylo1 <- buildNhoodGraph(mylo1)
saveRDS(mylo1, 'h5ad/R/milo_B_1.5.RDS')

load('h5ad/R/B_milo_prep_2.5.RData')
# 2.5 MIU vs Untreated (B)
mylo2 <- Milo(adata_no_knnx2)
milo_graph2 <- buildFromAdjacency(knn_adjacencyx2, k=50, is.binary=TRUE)
...
mylo2 <- countCells_(mylo2, meta.data = data.frame(colData(mylo2)), samples="sample_id")
mylo2 <- calcNhoodDistance(mylo2, d=50, reduced.dim = 'X_pca_harmony', use.assay = 'counts')
mylo2 <- buildNhoodGraph(mylo2)
saveRDS(mylo2, 'h5ad/R/milo_B_2.5.RDS')

load('h5ad/R/MNP_milo_prep_1.5.RData')
# 1.5 MIU vs Untreated (MNP)
mylo1 <- Milo(adata_no_knnx1)
milo_graph1 <- buildFromAdjacency(knn_adjacencyx1, k=50, is.binary=TRUE)
graph(mylo1) <- graph(milo_graph1)
mylo1 <- buildGraph(mylo1, k=50, d=50, reduced.dim="X_pca_harmony", BPPARAM = MulticoreParam(progressbar = TRUE))
...
mylo1 <- calcNhoodDistance(mylo1, d=50, reduced.dim = 'X_pca_harmony', use.assay = 'counts')
mylo1 <- buildNhoodGraph(mylo1)
saveRDS(mylo1, 'h5ad/R/milo_MNP_1.5.RDS')
load('h5ad/R/MNP_milo_prep_2.5.RData')
# 2.5 MIU vs Untreated (MNP)
mylo2 <- Milo(adata_no_knnx2)
milo_graph2 <- buildFromAdjacency(knn_adjacencyx2, k=50, is.binary=TRUE)
graph(mylo2) <- graph(milo_graph2)
mylo2 <- buildGraph(mylo2, k=50, d=50, reduced.dim="X_pca_harmony", BPPARAM = MulticoreParam(progressbar = TRUE))
...
mylo2 <- calcNhoodDistance(mylo2, d=50, reduced.dim = 'X_pca_harmony', use.assay = 'counts')
mylo2 <- buildNhoodGraph(mylo2)
saveRDS(mylo2, 'h5ad/R/milo_MNP_2.5.RDS')
...
mylo1 <- readRDS('h5ad/R/milo_B_1.5.RDS')
mylo2 <- readRDS('h5ad/R/milo_B_2.5.RDS')
...
save(res_summary1, res1, metadata1, file = 'h5ad/R/model_1/milo_B_1.5_results.RData')
...
save(res_summary2, res2, metadata2, file = 'h5ad/R/model_1/milo_B_2.5_results.RData')

```

# Export results for B cells
```R
library(miloR)
library(pbmcapply)
setwd('/lustre/scratch117/cellgen/team297/kt16/Ziad/scanpy')
...
load('h5ad/R/model_1/milo_B_1.5_results.RData')
load('h5ad/R/model_1/milo_B_2.5_results.RData')

b1.5_pvalue <- pbmclapply(res_summary1, function(x) x$coefficients[5,4])
b2.5_pvalue <- pbmclapply(res_summary2, function(x) x$coefficients[5,4])

b1.5_beta <- pbmclapply(res_summary1, function(x) x$coefficients[5,1])
b2.5_beta <- pbmclapply(res_summary2, function(x) x$coefficients[5,1])

da.res_b1.5 = data.frame(Nhood = as.numeric(1:nrow(nhoodCounts(mylo1))), beta = do.call(c, b1.5_beta), PValue = do.call(c, b1.5_pvalue))
da.res_b2.5 = data.frame(Nhood = as.numeric(1:nrow(nhoodCounts(mylo2))), beta = do.call(c, b2.5_beta), PValue = do.call(c, b2.5_pvalue))

# da.res$Nhood <- as.numeric(rownames(da.res))
message("Computing SpatialFDR")
da.res_b1.5$SpatialFDR <- graphSpatialFDR(x.nhoods=nhoods(mylo1), graph=miloR::graph(mylo1),
...
da.res_b2.5$SpatialFDR <- graphSpatialFDR(x.nhoods=nhoods(mylo2), graph=miloR::graph(mylo2),
...
saveRDS(da.res_b1.5, file = 'h5ad/R/model_1/milo_results_1.5vsuntreated_B.RDS')
saveRDS(da.res_b2.5, file = 'h5ad/R/model_1/milo_results_2.5vsuntreated_B.RDS')
```
...
mylo1 <- readRDS('h5ad/R/milo_MNP_1.5.RDS')
mylo2 <- readRDS('h5ad/R/milo_MNP_2.5.RDS')

res1 <- as.list(1:nrow(nhoodCounts(mylo1)))
res1 <- pbmclapply(res1, function(x) glmer.nb(nhoodCounts(mylo1)[x,] ~ sex + age + peak_trop + (1|study_id) + offset(log(colSums(nhoodCounts(mylo1)))) + treatment_group_1, data = metadata1, control=glmerControl(check.conv.singular = .makeCC(action = "ignore",  tol = 1e-2), optimizer="bobyqa", tol=1e-02)), mc.cores = parallel::detectCores())
res_summary1 <- pbmclapply(res1, summary)
save(res_summary1, res1, metadata1, file = 'h5ad/R/model_1/milo_MNP_1.5_results.RData')

res2 <- as.list(1:nrow(nhoodCounts(mylo2)))
res2 <- pbmclapply(res2, function(x) glmer.nb(nhoodCounts(mylo2)[x,] ~ sex + age + peak_trop + (1|study_id) + offset(log(colSums(nhoodCounts(mylo2)))) + treatment_group_1, data = metadata2, control=glmerControl(check.conv.singular = .makeCC(action = "ignore",  tol = 1e-2), optimizer="bobyqa", tol=1e-02)), mc.cores = parallel::detectCores())
res_summary2 <- pbmclapply(res2, summary)
save(res_summary2, res2, metadata2, file = 'h5ad/R/model_1/milo_MNP_2.5_results.RData')

```

# Export results for MNP
```R
library(miloR)
library(pbmcapply)
setwd('/lustre/scratch117/cellgen/team297/kt16/Ziad/scanpy')
...
load('h5ad/R/model_1/milo_MNP_1.5_results.RData')
load('h5ad/R/model_1/milo_MNP_2.5_results.RData')
...
saveRDS(da.res_b1.5, file = 'h5ad/R/model_1/milo_results_1.5vsuntreated_MNP.RDS')
saveRDS(da.res_b2.5, file = 'h5ad/R/model_1/milo_results_2.5vsuntreated_MNP.RDS')
```